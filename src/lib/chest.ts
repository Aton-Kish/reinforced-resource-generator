...
export class ChestGenerator {
  ...
  constructor(single: Jimp, left: Jimp, right: Jimp, material: Jimp) {
    if (!(single.getWidth() === 64 && single.getHeight() === 64)) {
      throw new Error('single image size must be 64x64')
    }

    if (!(left.getWidth() === 64 && left.getHeight() === 64)) {
      throw new Error('left image size must be 64x64')
    }

    if (!(right.getWidth() === 64 && right.getHeight() === 64)) {
      throw new Error('right image size must be 64x64')
    }
